...
#undef  PACKAGE
#define PACKAGE "lcspclient_rpc"

#include "../csp/usage.txt"

#include <ulib/application.h>

template <class T> class UClientLCSP : public URPCClient<T> {
public:

   // COSTRUTTORE

            UClientLCSP(UFileConfig* cfg) : UClient_Base(cfg), URPCClient<T>(cfg) {}
   virtual ~UClientLCSP()                                                          {}
   ...
   bool creatCA(const UString& n, uint32_t d, const UString& c) // 1
      ...
      if (URPCClient<T>::processRequest(m_LCSP_CA))
         {
   ...
   UString listCA() // 2
      ...
      if (URPCClient<T>::processRequest(m_LCSP_LIST_CA))
         {
   ...
   UString signP10(const UString& n, const UString& k, const UString& p) // 3
      ...
      if (URPCClient<T>::processRequest(m_LCSP_SIGN_P10))
         {
         result = URPCClient<T>::getResponse(); // Get the value of the element inside the response
         }
   ...
   UString signSPKAC(const UString& n, const UString& k, const UString& p) // 4
      ...
      if (URPCClient<T>::processRequest(m_LCSP_SIGN_SPKAC))
         {
         result = URPCClient<T>::getResponse(); // Get the value of the element inside the response
         }
   ...
   UString listCerts(const UString& name, uint32_t compress) // 5
      ...
      if (URPCClient<T>::processRequest(m_LCSP_LIST_CERTS))
         {
         result = URPCClient<T>::getResponse(); // Get the value of the element inside the response
         }
   ...
   bool removeCert(const UString& n, const UString& s) // 6
      ...
      if (URPCClient<T>::processRequest(m_LCSP_REMOVE_CERT))
         {
         int result = URPCClient<T>::getResponse().strtol(); // Get the value of the element inside the response
   ...
   bool zeroCerts(const UString& name) // 7
      ...
      if (URPCClient<T>::processRequest(m_LCSP_ZERO_CERTS))
         {
         int result = URPCClient<T>::getResponse().strtol(); // Get the value of the element inside the response
   ...
   bool emitCRL(const UString& name) // 8
      ...
      if (URPCClient<T>::processRequest(m_LCSP_EMIT_CRL))
         {
         int result = URPCClient<T>::getResponse().strtol(); // Get the value of the element inside the response
   ...
   UString getCRL(const UString& name) // 9
      ...
      if (URPCClient<T>::processRequest(m_LCSP_GET_CRL))
         {
         result = URPCClient<T>::getResponse(); // Get the value of the element inside the response
         }
   ...
   UString getCA(const UString& name) // 10
      ...
      if (URPCClient<T>::processRequest(m_LCSP_GET_CA))
         {
         result = URPCClient<T>::getResponse(); // Get the value of the element inside the response
         }
   ...
   bool revokeCert(const UString& n, const UString& s) // 11 
      ...
      if (URPCClient<T>::processRequest(m_LCSP_REVOKE_CERT))
         {
         int result = URPCClient<T>::getResponse().strtol(); // Get the value of the element inside the response
