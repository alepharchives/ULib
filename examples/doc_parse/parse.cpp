      ...
      if (stype->empty() == false)
         ...
         if      (U_STRNEQ(type, "image/"))                       icon = "/icons/img.png";
            ...
            if (U_STRING_FIND(*content, 0, "<?xml-stylesheet type=\"text/xsl\"") != U_NOT_FOUND)
               ...
               if (xsltproc_cmd == 0)
                  {
                  fd_stderr = UFile::open("/dev/null", O_WRONLY, PERM_FILE);
...
#     endif
   ...
   static void vprint(const char* format, ...)
      ...
      u_buffer_len = u__vsnprintf((u_buffer_ptr = u_buffer), U_MAX_SIZE_PREALLOCATE, format, argp);
      ...
      *stype = uMimeEntity->shortContentType();
      ...
      if (uMimeEntity->isBodyMessage()) (void) description.append(U_CONSTANT_TO_PARAM(" - BODY MESSAGE"));
         ...
         *pfile = uMimeEntity->getFileName();
         ...
         if (pfile->empty() == false) description.snprintf_add(" - Filename='%.*s'", U_STRING_TO_TRACE(*pfile));
         }
