...
protected:
   UTree<Element*> tree;
   UString description, buffer, p7content, filename;
   int type;
   bool binary;
   ...
   void    parse(const UString& content);
   void    parseMime(UMimeEntity& entity);
   void    setType(UTree<Element*>* pnode);
   void    buildTree(const UString& content);
   int     elementaryTypeOf(const UString& content);
