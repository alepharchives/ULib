...
DocumentClassifier::DocumentClassifier(const char* pathfile) : description(U_CAPACITY), buffer(U_CAPACITY)
{
   ...
   UString fileContent = UFile::contentOf(pathfile);
   ...
   type  = elementaryTypeOf(fileContent);
   ptree = &tree;
   ...
   if (type == Element::UNKNOWN)
      {
      if (binary == false) buildTree(fileContent);
      }
   else if (type == Element::PKCS7)
      {
      addElement(Element::PKCS7);

      if (binary == false) buildTree(p7content);
      }
}
...
int DocumentClassifier::elementaryTypeOf(const UString& content)
{
...
#  endif
      ...
      p7content = p7.getContent();

      binary = p7content.isBinary();
...
void DocumentClassifier::buildTree(const UString& content)
{
   U_TRACE(5, "DocumentClassifier::buildTree(%.*S)", U_STRING_TO_TRACE(content))
...
void DocumentClassifier::parse(const UString& content)
{
   U_TRACE(5, "DocumentClassifier::parse(%.*S)", U_STRING_TO_TRACE(content))
...
void DocumentClassifier::parseMime(UMimeEntity& entity)
{
   ...
   if (entity.isRFC822())
      ...
      UString content = tmp.getContent();
