...
#include <ulib/net/server/plugin/mod_ssi.h>
   ...
   static bool     user_exist, nodog_exist, check_user_agent;
   ...
   static bool db_store(int op, UString& new_record, const UString& old_record)
   {
      U_TRACE(5, "::db_store(%d,%.*S,%.*S)", op, U_STRING_TO_TRACE(new_record), U_STRING_TO_TRACE(old_record))
   
      U_INTERNAL_DUMP("new_record.size() = %u old_record.size() = %u", new_record.size(), old_record.size())
   
      if (op == RDB_REPLACE)
         {
         if (new_record.size() <= old_record.size())
            {
            uint32_t n = new_record.size();
            char* _ptr = old_record.data();
   
            (void) u__memcpy(_ptr, new_record.data(), n);
   
            (void) memset(_ptr + n, ' ', old_record.size() - n);
         ...
         new_record += U_STRING_FROM_CONSTANT("                                    "); // NB: padding with 36 char space...
         }
   ...
   public:
      ...
      static void callForAllEntry(bPF func);
      ...
      void fromVector(const UVector<UString>& _vec, uint32_t _index)
         ...
         UString vec1 =  _vec[_index+2],
                 vec2 =  _vec[_index+3];
   
                                                                  mac_white_list.clear();
         if (vec1.empty() == false && vec1 != *empty_list) (void) mac_white_list.split(U_STRING_TO_PARAM(vec1));
   
                                                                  group_accounts.clear();
         if (vec2.empty() == false && vec2 != *empty_list) (void) group_accounts.split(U_STRING_TO_PARAM(vec2));
         }
   ...
   static bool _findLabel()
   {
      U_TRACE(5, "::_findLabel()")
   ...
   static bool _findAccount()
   {
      U_TRACE(5, "::_findAccount()")
   
      if (ap_rec->group_accounts.empty() == false)
         {
         for (uint32_t i = 0, n = ap_rec->group_accounts.size(); i < n; ++i)
            {
            if (*account == ap_rec->group_accounts[i]) U_RETURN(false);
            }
         }
   ...
   static bool _findMAC()
   {
      U_TRACE(5, "::_findMAC()")
   
      if (ap_rec->mac_white_list.empty() == false)
         {
         for (uint32_t i = 0, n = ap_rec->mac_white_list.size(); i < n; ++i)
            {
            if (mac->equalnocase(ap_rec->mac_white_list[i])) U_RETURN(false);
            }
         }
   ...
   bool WiAuthAccessPoint::findAccount()
   {
      U_TRACE(5, "WiAuthAccessPoint::findAccount()")
   
      U_INTERNAL_ASSERT(nodog_exist)
      U_ASSERT_EQUALS(account->empty(), false)
   
      U_INTERNAL_DUMP("account = %.*S", U_STRING_TO_TRACE(*account))
   
      callForAllEntry(_findAccount);
   
      if (lindex == U_NOT_FOUND) U_RETURN(false);
   
      U_RETURN(true);
   }
   
   bool WiAuthAccessPoint::findMAC()
   {
      U_TRACE(5, "WiAuthAccessPoint::findMAC()")
   ...
   public:
      ...
      UString toString()
         ...
         x.snprintf("%u %10ld %u %.*s [ %.*s ]",
                    down,
                    last_info,
                    port,
                    U_STRING_TO_TRACE(_hostname),
                    U_STRING_TO_TRACE(ap_list));
      ...
      bool setValue(const UString& _address)
         ...
         if ((nodog_exist = (value.empty() == false)))
            {
            fromString();
      ...
      void setRecord(int _port, bool bdown = false, bool bnoconsume = false, UString* mac_white_list = 0, UString* group_accounts = 0)
         {
         U_TRACE(5, "WiAuthNodog::setRecord(%d,%b,%b,%p,%p)", _port, bdown, bnoconsume, mac_white_list, group_accounts)
         ...
         if (setValue(*address) == false)
            ...
            if (label->empty()) lindex = U_NOT_FOUND;
               ...
               {
               WiAuthAccessPoint::callForAllEntry(_findLabel);
         ...
         if (add_ap == false)
            ...
            if (lindex != U_NOT_FOUND)
               ...
               access_point.replace(lindex+2, mac_white_list ? *mac_white_list : *empty_list); // mac_white_list
               access_point.replace(lindex+3, group_accounts ? *group_accounts : *empty_list); // group_accounts
               }
            }
         else
            {
            ...
            access_point.push_back(*label);                                         // label
            access_point.push_back(mac_white_list ? *mac_white_list : *empty_list); // mac_white_list
            access_point.push_back(group_accounts ? *group_accounts : *empty_list); // group_accounts
   
            WiAuthAccessPoint::callForAllEntry(_findLabel);
   
            U_INTERNAL_ASSERT_DIFFERS(lindex, U_NOT_FOUND)
            }
   
         UString x = toString();
   
         if (db_store(op, x, value))
            {
   ...
   void WiAuthAccessPoint::callForAllEntry(bPF func)
   {
      U_TRACE(5, "WiAuthAccessPoint::callForAllEntry(%p)", func)
   
      lindex = U_NOT_FOUND;
   
      for (int32_t i = 0, n = nodog_rec->access_point.size(); i < n; i += NUM_ACCESS_POINT_ATTRIBUTE)
         {
   ...
   uint32_t WiAuthAccessPoint::findLabel()
   {
      U_TRACE(5, "WiAuthAccessPoint::findLabel()")
   
      U_INTERNAL_ASSERT(nodog_exist)
      U_ASSERT_EQUALS(label->empty(), false)
   
      callForAllEntry(_findLabel);
   
      if (lindex == U_NOT_FOUND)
         {
         U_LOGGER("*** LABEL(%.*s) NOT EXISTENT ON AP(%.*s) ***", U_STRING_TO_TRACE(*label), U_STRING_TO_TRACE(nodog_rec->_hostname));
         }
   ...
   public:
      ...
      UString getAP()
         ...
         x.snprintf("%.*s@%.*s:%.*s", U_STRING_TO_TRACE(_label), U_STRING_TO_TRACE(nodog), U_STRING_TO_TRACE(nodog_rec->_hostname));
      ...
      bool setNodogReference()
         ...
         UString x = (connected ? *address : nodog);
   ...
   static void callForAllAP(vPFprpr function)
   {
      U_TRACE(5, "::callForAllAP(%p)", function)
   
      _num_ap = 0;
   
      db_ap->callForAllEntry(function);
   
      num_ap = _num_ap;
   }
   
   static void callForAllAPSorted(vPFprpr function)
   {
      U_TRACE(5, "::callForAllAPSorted(%p)", function)
   
      _num_ap = 0;
   
      db_ap->callForAllEntrySorted(function);
   ...
   static void usp_init()
   {
      ...
      nodog_conf         = U_NEW(UString(UFile::contentOf("ap/nodog.conf")));
   ...
   static bool writeStatusAccessPoint()
   {
      ...
      UString riga(U_CAPACITY),
              vec1 = ap_rec->mac_white_list.join(" "),
              vec2 = ap_rec->group_accounts.join(" ");
      ...
      riga.snprintf(status_nodog_template->data(),
                    ...
                    U_STRING_TO_TRACE(vec1),
                    U_STRING_TO_TRACE(vec2),
                    VIRTUAL_HOST, size, ptr, U_STRING_TO_TRACE(nodog_rec->_hostname), U_STRING_TO_TRACE(*address), nodog_rec->port);
   ...
   static void setStatusNodog(UStringRep* key, UStringRep* data)
   {
      ...
      WiAuthAccessPoint::callForAllEntry(writeStatusAccessPoint);
   }
   ...
   static UString sendRequestToNodog(const char* fmt, ...)
   {
      ...
      url.snprintf("http://%.*s:%u/%.*s", U_STRING_TO_TRACE(*address), nodog_rec->port, U_STRING_TO_TRACE(buffer));
   ...
   static void checkAccessPoint(UStringRep* key, UStringRep* data)
   {
      ...
      time_t time_diff = u_now->tv_sec - nodog_rec->last_info;
   
      if (time_diff > (16L * 60L))
         {
   ...
   static bool setAccessPoint(bool localization)
   {
      ...
      uint32_t pos, end = UHTTP::processHTTPForm();
   ...
   ********************************/
   ...
   static void GET_edit_ap()
   {
      ...
      if (setAccessPoint(true) == false)
         ...
         if (ap_rec->mac_white_list.empty() == false) vec1 = ap_rec->mac_white_list.join(" ");
         if (ap_rec->group_accounts.empty() == false) vec2 = ap_rec->group_accounts.join(" ");
   ...
   static void GET_get_config()
   {
      ...
      UString key, body, pathname(U_CAPACITY);
   
      UHTTP::getFormValue(key, U_CONSTANT_TO_PARAM("key"), 0, 3, UHTTP::processHTTPForm());
      ...
      if (body.empty())
         ...
         body = UStringExt::substitute(*nodog_conf, U_CONSTANT_TO_PARAM("x.x.x.x"), U_STRING_TO_PARAM(*client_address));
   
         if (cfg.setData(body), cfg.processData()) body = cfg.getData();
         }
   ...
   loop:
            ...
            callForAllAP(checkAccessPoint);
   
            for (int32_t i = 0, n = vec->size(); i < n; i += 2)
               {
               if (nodog_rec->setValue((*vec)[i])) (void) sendRequestToNodog((*vec)[i+1]);
               }
   ...
   error:
      ...
      writeToLOG(auth_domain->c_str()); // NB: in this func we call getCounter() for chunk setting...
   ...
   static void GET_start_ap()
   {
      ...
      if (setAccessPoint(true))
         ...
         if (nodog_exist) callForAllUsers(quitUserConnected);
         }
   ...
   static void GET_status_ap()
   {
      ...
      if (virtual_name->equal(U_HTTP_VHOST_TO_PARAM) &&
          setAccessPoint(true))
         {
         UString result = sendRequestToNodog("status?label=%.*s", U_STRING_TO_TRACE(*label));
   ...
   static void GET_status_network()
   {
      ...
      if (*client_address != *ip_server) u_http_info.nResponseCode = HTTP_BAD_REQUEST;
         ...
         buffer.snprintf(_template.data(), num_ap, users_connected);
   
         (void) output->insert(0, buffer);
   ...
   static void GET_status_nodog()
   {
      ...
      if (*client_address != *ip_server) u_http_info.nResponseCode = HTTP_BAD_REQUEST;
         ...
         callForAllAPSorted(setStatusNodog);
   
         buffer.snprintf(_template.data(), num_ap, VIRTUAL_HOST);
   ...
   static void GET_view_user()
   {
      ...
      if (*ip_server == *client_address)
         ...
         UString buffer(U_CAPACITY), user,
   ...
   static void POST_edit_ap()
   {
      ...
      int ap_port = 5280;
      uint32_t end = UHTTP::processHTTPForm();
      UString up, consume, mac_white_list, group_accounts;
      ...
      UHTTP::getFormValue(mac_white_list, U_CONSTANT_TO_PARAM("ap_mac_white_list"), 0,  7, end);
      UHTTP::getFormValue(group_accounts, U_CONSTANT_TO_PARAM("ap_group_accounts"), 0,  9, end);
      ...
      nodog_rec->setRecord(ap_port,
                                up.empty(),
                           consume.empty(),
                           (mac_white_list.empty() ? 0 : &mac_white_list),
                           (group_accounts.empty() ? 0 : &group_accounts));
   ...
   static void POST_uploader()
   {
      ...
      if (UHTTP::processHTTPForm())
         ...
         if (source.empty() == false)
            {
            UString content = UFile::contentOf(source);
