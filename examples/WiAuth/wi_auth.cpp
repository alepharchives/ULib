...
#include <ulib/net/server/plugin/mod_ssi.h>
   ...
   public:
      ...
      void setDown(bool bdown)
         ...
         char c     = ((down = bdown) ? '1' : '0');
         char* _ptr = value.data();
   
         U_INTERNAL_ASSERT(u__isspace(_ptr[1]))
         U_INTERNAL_ASSERT_EQUALS(_ptr[0], bdown ? '0' : '1')
                                  _ptr[0] = c;
         }
   
      void setValue(const UString& _address)
         {
      ...
      void setRecord(int ap_port)
         {
         U_TRACE(5, "WiAuthNodog::setRecord(%d)", ap_port)
         ...
         if (nodog_exist)
            ...
            if ( ap->empty() == false &&
                *ap != hostname)
               {
               U_LOGGER("*** AP HOSTNAME: (%.*s=>%.*s) NOT EQUAL ***", U_STRING_TO_TRACE(*ap), U_STRING_TO_TRACE(hostname));
               }
   ...
   public:
      ...
      UString value, _ip;
      time_t login_time, last_modified;
      uint64_t traffic_done, traffic_available, traffic_consumed;
      time_t time_done, time_available, time_consumed;
      UString _user, _auth_domain, _mac, _gateway, _policy, _token, nodog;
      uint32_t access_point_index, group_account, agent;
      ...
      UString toString()
         ...
         x.snprintf("%.*s %u "
                    ...
                    access_point_index, consume, group_account, agent,
      ...
      void resetDailyConsume()
         {
         U_TRACE(5, "WiAuthUser::resetDailyConsume()")
      ...
      void updateCounter(time_t _logout, time_t _connected, uint64_t _traffic)
         ...
         // 172.16.1.172 1 1346679913 1346679913        8            12736          4      7200         12736   314572800 
         // 0 1 0 3821975508 "stefano casazza" PASS_AUTH 00:14:a5:6e:9c:cb 172.16.1.254:5280 DAILY 1508283900 10.8.1.2
      ...
      bool setNodogReference()
         ...
         if (check_user_agent &&
            ...
            {
            U_ASSERT_DIFFERS(*client_address, nodog)
   
            U_LOGGER("*** UID(%.*s) BIND WITH AP(%.*s) HAS DIFFERENT AGENT ***", U_STRING_TO_TRACE(*uid), U_STRING_TO_TRACE(nodog));
            }
   
         nodog_rec->setValue(nodog);
   
         if (nodog_exist == false)
            {
            U_LOGGER("*** UID(%.*s) BIND WITH AP(%.*s) NOT EXISTENT ***", U_STRING_TO_TRACE(*uid), U_STRING_TO_TRACE(nodog));
      ...
      void store(int op)
         ...
         U_INTERNAL_DUMP("x.size() = %u value.size() = %u", x.size(), value.size())
         ...
         if (op == RDB_REPLACE)
            ...
            if (x.size() <= value.size())
               ...
               (void) u__memcpy(_ptr, x.data(), n);
   
               (void) memset(_ptr+n, ' ', value.size() - n);
            ...
            {
            U_INTERNAL_ASSERT_EQUALS(op, RDB_INSERT)
   
            x += U_STRING_FROM_CONSTANT("            "); // NB: padding with 12 char space...
            }
      ...
      bool setRecord()
         ...
         if (user_exist == false)
            ...
            if (content.empty()) _user = U_STRING_FROM_CONSTANT("anonymous");
            else
               {
         ...
         (void) setNodogReference();
   
         consume            = true;
         access_point_index = group_account = 0;
         ...
         for (; i < n; ++i)
            ...
            if (*label == ap_rec->label)
               ...
               // NB: check for group account
   
               if (ap_rec->group_accounts.empty() == false)
                  {
                  UString account(100U);
   
                  account.snprintf("%.*s:%.*s", U_STRING_TO_TRACE(*uid), U_STRING_TO_TRACE(*password));
   
                  for (uint32_t j = 0, k = ap_rec->group_accounts.size(); j < k; ++j)
                     {
                     if (ap_rec->group_accounts[j] == account)
                        {
                        group_account = 1;
               ...
               if (group_account) consume = false;
               else               consume = (ap_rec->noconsume == false);
   ...
   static void countUser(UStringRep* key, UStringRep* data)
   {
      U_TRACE(5, "::countUser(%.*S,%.*S)", U_STRING_TO_TRACE(*key), U_STRING_TO_TRACE(*data))
      ...
      U_INTERNAL_DUMP("num_user = %u", num_user)
   }
   ...
   static void usp_init()
   {
      ...
      if (UServer_Base::bssl == false)
         {
         db_user->callForAllEntry(countUser);
   
         U_SRV_LOG("db initialization of wi-auth users WiAuthUser.cdb %s: num_user %u", result ? "success" : "FAILED", num_user);
      ...
      message_page_template = U_NEW(UString(cache->contentOf("%.*s/message_page.tmpl", U_STRING_TO_TRACE(*dir_template))));
      ...
      *ap = user_rec->getAP();
      ...
      (void) U_SYSCALL(write, "%d,%p,%u", file_LOG->getFd(), buffer,
                           u__snprintf(buffer, sizeof(buffer),
                              ...
                              op ? op : user_rec->_auth_domain.c_str(),
   ...
   static void resetUserDailyConsume(UStringRep* key, UStringRep* data)
   {
      U_TRACE(5, "::resetUserDailyConsume(%.*S,%.*S)", U_STRING_TO_TRACE(*key), U_STRING_TO_TRACE(*data))
   
      user_rec->value._assign(data);
   
      user_rec->fromString();
   
      if (user_rec->_policy == *policy) user_rec->resetDailyConsume();
   }
   ...
   static bool setAccessPointReference()
   {
      ...
      if (UHTTP::form_name_value->size() < 14)
         {
   error:
         ap->clear();
   
         ap_ref->snprintf("default", 0);
      ...
      if (setAccessPointAddress() == false) goto error; 
   ...
   static bool setAccessPoint(bool localization, bool start)
   {
      ...
      UHTTP::getFormValue(public_address, U_CONSTANT_TO_PARAM("public"), 0, 3, end);
      ...
      nodog_rec->setRecord(ap_port);
   
      if (start)
         {
         UString pid(100U);
   
         UHTTP::getFormValue(pid, U_CONSTANT_TO_PARAM("pid"), 0, 5, end);
   ...
   static bool askToLDAP(UString* pinput, const char* title_txt, const char* message, const char* fmt, ...)
   {
      ...
      */
      ...
      if (result <= 0)
         ...
         USSIPlugIn::setMessagePage(*message_page_template, title_txt, message);
   ...
   static bool askNodogToLogoutUser()
   {
      ...
      if (result.empty() == false &&
          U_IS_HTTP_ERROR(client->responseCode()) == false)
         {
         U_ASSERT_DIFFERS(U_STRING_FIND(result, 0, "DENY"), U_NOT_FOUND)
         }
   #endif
      ...
      if (checkIfUserConnected()) U_RETURN(false);
   
      U_RETURN(true);
   }
   
   static void askNodogToCheckUser()
   {
      U_TRACE(5, "::askNodogToCheckUser()")
   ...
   static bool checkLoginRequest()
   {
      ...
      // $9  -> timeout
      // $10 -> token
      // $11 -> ap (with localization => '@')
   
      UHTTP::getFormValue(*ap, U_CONSTANT_TO_PARAM("ap"), 0, 21, 22);
   
      if (setAccessPointAddress()) U_RETURN(true);
   ...
   static void checkLogoutRequest()
   {
      ...
      if (UHTTP::isHttpGET()) uid->clear();
         ...
         UHTTP::getFormValue(*uid, U_CONSTANT_TO_PARAM("uid"), 0, 1, UHTTP::form_name_value->size());
         }
   ...
   error:
      ...
      if (UHTTP::isHttpGET())
         ...
         USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/ringraziamenti.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                           "Firenze WiFi", 0, 0,
                                           uid->c_str(), _time_chunk->data(), _traffic_chunk->data());
         }
      else
         {
         U_http_is_connection_close = U_YES;
   
         USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/logout_notify.tmpl", U_STRING_TO_TRACE(*dir_template)),
   ...
   static void isLogged()
   {
      ...
      *ap = user_rec->getAP();
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/logged.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                        "Firenze WiFi", 0, 0,
   ...
   static void GET_error_ap()
   {
      ...
      if (setAccessPoint(true, false))
         {
         U_LOGGER("*** ON AP(%.*s) THE FIREWALL IS NOT ALIGNED ***", U_STRING_TO_TRACE(*ap));
         }
   ...
   static void GET_get_config()
   {
      ...
      UHTTP::getFormValue(key, U_CONSTANT_TO_PARAM("key"), 0, 3, UHTTP::form_name_value->size());
   
      ap_ref->snprintf("%.*s", U_STRING_TO_TRACE(key));
   
      UFileConfig cfg;
      UString body, pathname(U_CAPACITY);
   ...
   static void GET_get_users_info()
   {
      ...
      if (*client_address != *ip_server) u_http_info.nResponseCode = HTTP_BAD_REQUEST;
         ...
         if (setAccessPoint(true, false)) askNodogToCheckUser();
   ...
   static void GET_info()
   {
      ...
      uint32_t end, num_args = UHTTP::form_name_value->size() / 2;
      ...
      for (int32_t i = 0, n = num_args / 2; i < n; i += 8)
         ...
         UHTTP::getFormValue(*gateway, U_CONSTANT_TO_PARAM("gateway"), i, i+5, end);
         ...
         if ((*ip      != user_rec->_ip)  ||
            ...
            {
            U_LOGGER("*** INFO DIFFERENCE: AP(%.*s) CLIENT(%.*s) IP(%.*s=>%.*s) MAC(%.*s=>%.*s) GATEWAY(%.*s=>%.*s) ***",
   ...
   static void GET_logged_login_request()
   {
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/logged_login_request.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                        "Firenze WiFi", 0, 0,
                                        0);
   }
   ...
   static void GET_login() // MAIN PAGE
   {
   ...
   #ifndef U_MANAGED_BY_MAIN_BASH
      if (setAccessPointReference() &&
          (uid->clear(), checkIfUserConnected()))
         {
         isLogged();
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/login.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                        "Firenze WiFi", 0, 0,
   ...
   static void GET_login_request()
   {
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/login_request.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                        "Firenze WiFi", 0, 0,
   ...
   static void GET_login_validate()
   {
      ...
      if (UHTTP::form_name_value->size() < 22 ||
   ...
   error:
      ...
      UHTTP::getFormValue(*mac, U_CONSTANT_TO_PARAM("mac"),      0, 1, 22);
      UHTTP::getFormValue(*ip,  U_CONSTANT_TO_PARAM("ip"),       0, 3, 22);
      UHTTP::getFormValue(*uid, U_CONSTANT_TO_PARAM("redirect"), 0, 5, 22);
      ...
      UHTTP::getFormValue(*policy,             U_CONSTANT_TO_PARAM("policy"),      0,  7, 22);
      UHTTP::getFormValue(*auth_domain,        U_CONSTANT_TO_PARAM("auth_domain"), 0,  9, 22);
      UHTTP::getFormValue(*_time_available,    U_CONSTANT_TO_PARAM("max_time"),    0, 11, 22);
      UHTTP::getFormValue(*_traffic_available, U_CONSTANT_TO_PARAM("max_traffic"), 0, 13, 22);
      UHTTP::getFormValue(*gateway,            U_CONSTANT_TO_PARAM("gateway"),     0, 15, 22);
      UHTTP::getFormValue(*token,              U_CONSTANT_TO_PARAM("token"),       0, 19, 22);
   ...
   static void GET_logout_page()
   {
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/logout_page.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                        "Firenze WiFi", 0, 0,
                                        0);
   }
   ...
   error:
         ...
         if ((*ip      != user_rec->_ip)    ||
             ...
             (*gateway != user_rec->_gateway))
            {
            U_LOGGER("*** POSTLOGIN DIFFERENCE: IP(%.*s=>%.*s) MAC(%.*s=>%.*s) TOKEN(%.*s=>%.*s) GATEWAY(%.*s=>%.*s) ***",
                        U_STRING_TO_TRACE(*ip),      U_STRING_TO_TRACE(user_rec->_ip),
                        U_STRING_TO_TRACE(*mac),     U_STRING_TO_TRACE(user_rec->_mac),
         ...
         USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/postlogin.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                           "Firenze WiFi", "<script type=\"text/javascript\" src=\"js/logout_popup.js\"></script>", buffer.data(),
                                           ptr1, ptr2, ptr2);
         }
      else if (num_args == 1)
         {
         ...
         USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/logout_popup.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                           "Logout popup", 0, 0,
                                           _ptr, _ptr);
         }
   #endif
   ...
   static void GET_recovery()
   {
      ...
      if (*ip_server == *client_address)
         ...
         UHTTP::getFormValue(*uid, U_CONSTANT_TO_PARAM("user"), 0, 1, UHTTP::form_name_value->size());
   
         if (checkIfUserConnected()) (void) askNodogToLogoutUser();
   
         u_printf2(file_RECOVERY->getFd(), "%6D %.*s \"%.*s\"\n", U_STRING_TO_TRACE(*uid), U_STRING_TO_TRACE(user_rec->_user));
   ...
   static void GET_registrazione()
   {
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/registrazione.tmpl", U_STRING_TO_TRACE(*dir_template)),
   ...
   static void GET_reset_policy()
   {
      ...
      if (*client_address != *ip_server) u_http_info.nResponseCode = HTTP_BAD_REQUEST;
         ...
         *policy = U_STRING_FROM_CONSTANT("DAILY");
         ...
         db_user->callForAllEntry(resetUserDailyConsume);
   ...
   static void GET_unifi()
   {
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/unifi_page.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                        "Firenze WiFi", 0, 0,
                                        url_banner_ap->data(), help_url->data(), wallet_url->data(),
   ...
   static void GET_unifi_login_request()
   {
      ...
      USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/unifi_login_request.tmpl", U_STRING_TO_TRACE(*dir_template)),
                                        "Firenze WiFi", 0, 0,
                                        0);
   }
   ...
   static void GET_view_user()
   {
      ...
      if (*ip_server == *client_address)
         ...
         UString buffer(U_CAPACITY),
                 _template = cache->contentOf("%.*s/view_user.tmpl", U_STRING_TO_TRACE(*dir_template));
         ...
         buffer.snprintf(_template.data(),
                         ptr, uid->c_str(),
                         ptr3, ptr4,
                         WA_PASSWORD.c_str(),
                         not_after.data(),
         ...
         (void) UFile::writeTo(outfile, buffer);
   ...
   static void POST_login_request()
   {
      ...
      // NB: time_available, traffic_available sono valori di *** CONSUMO ***
   
      if (table->empty() == false)
         {
         *_time_available    = (*table)["waTime"];    // waTime: 7200
         *_traffic_available = (*table)["waTraffic"]; // waTraffic: 314572800
         }
      else
         {
         U_ASSERT_EQUALS(policy->empty(), false)
         ...
         pathname.snprintf("%.*s/policy/%.*s", U_STRING_TO_TRACE(*dir_root), U_STRING_TO_TRACE(*policy));
   
         content = UFile::contentOf(pathname);
   
         if (UFileConfig::loadProperties(*table, content.data(), content.end()))
            {
            *_time_available    = (*table)["MAX_TIME"];    // MAX_TIME=7200
            *_traffic_available = (*table)["MAX_TRAFFIC"]; // MAX_TRAFFIC=314572800
            }
         }
   
      table->clear();
   
      UString signed_data = UDES3::signData("uid=%.*s&policy=%.*s&auth_domain=%.*s&max_time=%.*s&max_traffic=%.*s",
                                       U_STRING_TO_TRACE(*uid),
   ...
   static void POST_registrazione()
   {
      ...
      UString nome, cognome, luogo_di_nascita, data_di_nascita, email,
              cellulare_prefisso, telefono_cellulare, password_conferma;
      ...
      if (*password != password_conferma) USSIPlugIn::setMessagePage(*message_page_template, "Conferma Password errata", "Conferma Password errata");
         ...
         USSIPlugIn::setAlternativeInclude(cache->contentOf("%.*s/post_registrazione.tmpl", U_STRING_TO_TRACE(*dir_template)),
