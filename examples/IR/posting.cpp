...
U_NO_EXPORT bool UPosting::findCurrentDocIdOnPosting(UStringRep* value)
{
   ...
   ptr = (const char*)u_find(posting->data(), posting->size(), ptr_cur_doc_id, sizeof(cur_doc_id));

   if (ptr == 0) return;
#else
   ptr = (const char*) UCDB::getPtrPattern();

   U_ASSERT_EQUALS(ptr,(const char*)u_find(posting->data(), posting->size(), ptr_cur_doc_id, sizeof(cur_doc_id)))
#endif

   // check for collision...

   bool result = checkEntry(ptr, posting->data(), posting->size());
...
U_NO_EXPORT void UPosting::del()
{
   ...
   if (isOneEntry())
      ...
      if (result != 0)
         {
         U_ERROR("del(): error<%d> on operation remove word<%.*s> from index database...",
                     result, U_STRING_TO_TRACE(*word));
         }
...
U_NO_EXPORT void UPosting::checkDocument(UStringRep* word_rep, UStringRep* value)
{
...
#  ifndef U_OPTIMIZE
      if (cdb_words->ignoreCase() == false &&
          u_find(content->data(), content->size(), word->data(), word->size()) == 0) return;
#  endif
...
U_NO_EXPORT void UPosting::checkAllEntry(UStringRep* word_rep, UStringRep* value)
{
...
void UPosting::checkAllEntry()
{
   ...
   cdb_words->callForAllEntry(checkAllEntry);
}

// SUBSTITUTE DOCUMENT

U_NO_EXPORT void UPosting::substitute(UStringRep* word_rep, UStringRep* value)
{
...
#ifdef U_COMPRESS_ENTRY
#  define U_CHECK_DOCUMENT checkDocument
#else
#  define U_CHECK_DOCUMENT checkDocument,str_cur_doc_id
#endif
...
void UPosting::setDocID(int32_t op)
{
   ...
   if (tbl_name)
      ...
      if (op == 1) // sub
         ...
         ((URDB*)cdb_words)->callForAllEntry(substitute);
         }
      else if (op == 3) // check
         {
         ...
         cdb_words->callForAllEntry(U_CHECK_DOCUMENT);
         }
      }
}
...
bool UPosting::findDocID(UStringRep* word_rep)
{
   ...
   if (property[i].is_quoted)
      ...
      bool result = (entry.empty() == false && u_find(entry.data(), entry.size(), ptr_cur_doc_id, sizeof(cur_doc_id)));
...
void UPosting::callForPosting(vPF function, bool is_space)
{
   ...
   if (is_space) // composite word
      ...
      if (posting->empty() == false) callForPostingAndSetFilename(function);
      }
}
...
U_NO_EXPORT void UPosting::print(UStringRep* word_rep, UStringRep* value)
{
...
U_NO_EXPORT void UPosting::printDocName(UStringRep* doc_id, UStringRep* doc_name)
{
...
void UPosting::printDB(ostream& s)
{
   ...
   if (tbl_words)
      {
      tbl_words->callForAllEntry((vPFprpv)print);
      }
   else
      {
      U_INTERNAL_ASSERT_POINTER(cdb_names)
      ...
      cdb_names->callForAllEntry(printDocName);
      cdb_words->callForAllEntry(print);
      }
