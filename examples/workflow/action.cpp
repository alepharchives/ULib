...
bool Action::sendHttpPostRequest(const UString& url, const UString& body, const char* content_type, const char* expected)
{
   ...
   if (response.empty() ||
       client_http.responseCode() != HTTP_OK)
      {
      ok = false;
   ...
   else if (ok && strncmp(response.data(), expected, strlen(expected)))
      {
      ok = false;
...
bool Action::sendEmail()
{
   ...
   if (ok)
      ...
      if (*state == '\0') buffer.snprintf(emailBodyForm.data(), customer, installation, uid);
         ...
         msg.add(UMimeMultipartMsg::section(buffer, "", UMimeMultipartMsg::AUTO));
   ...
   if (ok == false)
      {
      response.setBuffer(100U);
      response.snprintf("Email notification to '%.*s' failed: %s", U_STRING_TO_TRACE(toAddress), emailClient.status());
      }
