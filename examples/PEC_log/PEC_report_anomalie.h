...
#define U_ERROR_MESSAGE_OUT_OF_TTL "TTL MESSAGE <%ld> OUT OF RANGE <%ld> - id = <%.*s>"
...
public:
   ...
   bool isOld()
      ...
      U_INTERNAL_DUMP("now = %ld, start = %ld, ttl = %ld", now, start, ttl)
...
public:
   ...
   static void reportAnomalie(UStringRep* key, void* elem);
   ...
   static void reportMaxTTL()
      ...
      if (Messaggio::max_ttl)
         {
         (void) sprintf(buffer, " - max TTL: %ld day(s) for message <%.*s>",
