...
U_EXPORT main (int argc, char* argv[])
{
   ...
   result = x.yesno("Hello, this is my first dialog program");

   x.setSize(16, 51);

   UString init = U_STRING_FROM_CONSTANT("bucaiolo");

   result = x.inputbox("Hi, this is an input dialog box. You can use\n"
   ...
   result = x.passwordbox("Hi, this is an password dialog box. You can use\n"
                       "this to ask questions that require the user\n"
                       "to input a string as the answer. You can\n"
                       "input strings of length longer than the\n"
                       "width of the input box, in that case, the\n"
...
// U_ASSERT( memcmp(selected, selected_result, sizeof(selected[7])) == 0 )
   ...
   result = x.radiolist("Hi, this is a radiolist box. You can use this to\n"
                        "present a list of choices which can be turned on or\n"
                        "off. If there are more items than can fit on the\n"
                        "screen, the list will be scrolled. You can use the\n"
                        "UP/DOWN arrow keys, the first letter of the choice as a\n"
   ...
   index = x.menu("Hi, this is a menu box. You can use this to\n"
   ...
   result = x.fselect(filepath, "Please choose a file");
   ...
   if (UDialog::isXdialog())
      ...
      index = x.tree("treeview box demo", items3, items_depth, item_tags, items_status, items_help, 0, "TREE VIEW BOX");
      }
}
