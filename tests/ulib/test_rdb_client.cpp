...
static void transaction(URDBClient<UTCPSocket>& rdb)
{
   ...
   if (rdb.beginTransaction())
      ...
      U_ASSERT( rdb[key1].empty() == true )
      U_ASSERT( rdb[key2].empty() == true )
      U_ASSERT( rdb[key3].empty() == true )
      }
}
...
U_EXPORT main(int argc, char* argv[])
{
   ...
   if (x.setHostPort(U_STRING_FROM_CONSTANT("localhost"), 80) && x.connect())
      {
   ...
   #define LDATA "\n"
      ...
      if (x.connect())
         ...
         UString _data = U_STRING_FROM_CONSTANT("valore_di_prova");

         x.store(_key, _data, RDB_INSERT);
         x.store(_key, _data, RDB_REPLACE);

         value = x[_key];

         U_ASSERT( value == _data )
