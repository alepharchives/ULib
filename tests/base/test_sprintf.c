...
#  define   printf(fmt,args...)               write(STDOUT_FILENO,ubuffer,u_snprintf(ubuffer,sizeof(ubuffer),fmt,args))
#  define  sprintf(buf,fmt,args...)           u_snprintf(buf,sizeof(buf),fmt,args)
#  ifdef __MINGW32__
#     define _snprintf(buf,szbuf,fmt,args...) u_snprintf(buf,szbuf,fmt,args)
#  else
#     define  snprintf(buf,szbuf,fmt,args...) u_snprintf(buf,szbuf,fmt,args)
#  endif
#else
#  include <stdio.h>
#  include <stdlib.h>
...
test2 (void)
{
   ...
   n = snprintf(buf2, sizeof(buf2) - 10, "%.999u", 10);
...
test3 (void)
{
   ...
   sprintf (buf, "%07llo", 040000000000ll);
