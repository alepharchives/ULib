...
static void do_cipher(int operation)
{
  ...
  (void) setmode(1, O_BINARY);
   ...
   while (1)
      ...
      if (operation == U_DECODE) ebuflen = u_url_decode(buf, readlen, ebuf, false);
      else
         {
         if (operation == U_ENCODE) ebuflen = u_url_encode(buf, readlen, ebuf, 0);
         else
            {
            char* new_buf;
            char* ptr = (char*)buf;
            ...
            ebuflen = u_url_encode((unsigned char*)new_buf, ptr - new_buf, ebuf, 0);
            }
         }
