...
TO   = (argv[5]?atoi(argv[5]):1000);
   ...
   //exit(0);
	...
	sprintf(buf, "%s/test.txt", IP);
   FILE *fo = fopen(buf, "w+b");
   if(!fo)
   {
   ...
   if (argv[6] &&
		 strcmp(argv[1], "localhost") == 0) // any server process name provided on command line?
   {
      sprintf(buf, "> Collecting CPU/RAM stats for server '%s'", argv[6]);
      fputs(buf, fo);
      printf("%s", buf);
      char str[80];
      nbr_pids = pidsof(argv[6], &pids);
      if(nbr_pids)
      ...
      /* THIS COMMAND LINE WORKS IN A TERMINAL BUT FAILS HERE... (tip?)
      char version[256] = {0};
      proc_ver(argv[6], version, sizeof(version) - 1);
      if(*version)
      {
         printf("version: %s\n", version);
...
/*             "\"http://" IP ":" PORT URL "\"\n\n", */
   ...
   for(i = FROM; i <= TO; i += STEP)
   {
      ...
      sprintf(str, "ab -n 10000 -c %d -S -d -t 1 %s "
      ...
      sprintf(str, "httperf --server=" IP " --port=" PORT " "
               "--rate=%d "
#ifdef KEEP_ALIVES               
               "--num-conns=%d --num-calls 10000 " // KEEP-ALIVES
#else               
               "--num-conns=10000 --num-calls 1 " // NO Keep_Alives
#endif               
               "--timeout 5 --hog --uri=\""
               URL "\""
#ifdef _WIN32                    
               ...
               , i?i:1, i?i:1);
#elif defined LIGHTY_WEIGHTTP
      sprintf(str, "weighttp -n 10000 -c %d -t %u %s "
						 "-H \"Accept-Encoding: gzip,deflate\" " // HTTP compression
                /* "\"http://" IP ":" PORT URL "\"" */
                   "\"http://%s"  ":%d"  "%s" "\""
      ...
      for(max_rps = 0, ave_rps = 0, min_rps = 0xffff0, j = 0; j < ITER; j++)
      {
         ...
         *buf = 0;
         ...
         if(*buf)
         {
            ...
            if(p) // "Reply status: 1xx=0 2xx=10000 3xx=0 4xx=0 5xx=0"
            {
               char *n;
