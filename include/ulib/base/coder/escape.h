...
#ifndef ULIBASE_ESCAPE_H
#define ULIBASE_ESCAPE_H 1
...
extern U_EXPORT uint32_t      u_sprintc(char* restrict buffer, unsigned char c);
extern U_EXPORT unsigned char u_escape_decode_ptr(const char** restrict s);

extern U_EXPORT uint32_t u_escape_encode(const unsigned char* restrict s, uint32_t n,          char* restrict result, uint32_t max_length);
