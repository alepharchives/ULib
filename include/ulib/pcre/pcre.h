...
/**
   ...
   @see UPCRE(const UString& expression, int flags)
*/
...
public:
   ...
   */
   ...
   void set(const UString& expression,        int  flags);
   ...
   */
   ...
   bool search(const UString& stuff,                      int offset = 0, int options = 0, bool bresultset = true)
      { return search(U_STRING_TO_PARAM(stuff), offset, options, bresultset); }
...
protected:
   ...
   bool global_t, replace_t;    /* internal compile flags, used by replace() and split() */
...
private:
   void        zero(int flags) U_NO_EXPORT;                    /* init pointers and counters */
