...
public:
   ...
   virtual void encodeArgument(const UString& argName, const UString& argType, const UString& argContent)
      ...
      if (   argName.empty() == false) arg.push(argName);
      if (   argType.empty() == false) arg.push(argType);
      if (argContent.empty() == false) arg.push(argContent);
      }
   ...
   virtual UString encodeFault(URPCFault* fault)
      ...
      U_RPC_ENCODE_RES(URPCMethod::str_fault->data(), buffer);
