...
static int add_file_to_zip(int jfd, int ffd, const char* fname, struct stat* statbuf)
{
   ...
   (void) u__strcpy(ze->filename, fname);
...
static int add_to_zip(int fd, const char* file)
{
   ...
   if (S_ISDIR(statbuf.st_mode))
      ...
      (void) u__strcpy(ze->filename, fullname);
      ze->filename[nlen] = '\0';
   ...
   for (;;)
      ...
      if (strchr((const char*)filename, '/') != 0 && handle)
         ...
         for (;;)
            ...
            (void) u__strncpy(tmp_buff, (const char *)filename, (idx - filename));

            tmp_buff[(idx - filename)] = '\0';

            U_INTERNAL_TRACE("checking the existance of %s", tmp_buff)
