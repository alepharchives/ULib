...
int URingBuffer::write(const char* buf, int len, bool pkt)
{
   ...
   if (split > 0)
      {
      (void) u_memcpy(ptrd + ptr->pwrite, buf, split);
   ...
   (void) u_memcpy(ptrd + ptr->pwrite, buf, todo);
...
int URingBuffer::read(int readd, char* buf, int len)
{
   ...
   if (split > 0)
      {
      (void) u_memcpy(buf, ptrd + ptr->pread[readd], split);
   ...
   (void) u_memcpy(buf, ptrd + ptr->pread[readd], todo);
