...
bool UFtpClient::createPassiveDataConnection()
{
   ...
   if (response == FTP_ENTERING_PASSIVE_MODE &&
      ...
      if (pasv.USocket::socket(SOCK_STREAM) &&
          pasv.USocket::connectServer(USocket::remoteIPAddress(), port))
         {
         U_RETURN(true);
         }
      }
...
size_t UFtpClient::getFileSize(const UIPAddress& ip, const UString& path)
{
   ...
   if (connectServer(ip) == false) U_RETURN(0); // connect with ftp server failed...
