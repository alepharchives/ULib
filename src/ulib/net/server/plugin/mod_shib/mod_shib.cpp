...
UShibTarget::UShibTarget()
{
   ...
   U_INTERNAL_DUMP("protocol = %S hostname = %S port = %d uri = %S content_type = %S remote_addr = %S method = %S",
                     protocol, hostname, port, uri, content_type, remote_addr, method)

   ShibTarget::init(protocol, hostname, port, uri, content_type, remote_addr, method); 
}
   ...
   // UShibTarget::protocol    = "http";
      ...
      if (mode == 0)
         ...
         if (UShibTarget::location == 0) mode = -1;
            ...
            UHTTP::setHTTPRedirectResponse(false, ext, UShibTarget::location, u_str_len(UShibTarget::location));
