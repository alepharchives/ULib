...
UIptAccount::UIptAccount(bool bSocketIsIPv6) : USocket(bSocketIsIPv6)
{
   ...
   data      = U_MALLOC(IPT_ACCOUNT_MIN_BUFSIZE);
...
bool UIptAccount::readEntries(const char* table, bool dont_flush)
{
   ...
   (void) strncpy(handle->name, table, ACCOUNT_TABLE_NAME_LEN-1);
   ...
   if (data_size < new_size ||
      ...
      data = U_MALLOC((data_size = new_size));
      }

   // Copy data from kernel

   (void) U_SYSCALL(memcpy, "%p,%p,%u", data, handle, sizeof(struct ipt_acc_handle_sockopt));
