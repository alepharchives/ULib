...
const char* USSHSocket::getError()
{
   U_TRACE(0, "USSHSocket::getError()")
...
int USSHSocket::recv(void* pBuffer, int iBufferLen)
{
   ...
   if (iBytesRead > 0) u_mem_cpy(pBuffer, buffer_get(buffer), iBytesRead);

#ifdef DEBUG
        if (iBytesRead > 0) U_INTERNAL_DUMP("BytesRead(%d) = %#.*S", iBytesRead, iBytesRead, CAST(pBuffer))
   else if (iBytesRead < 0) U_DUMP("status = %.*S", 512, status(iBytesRead))
#endif

   U_RETURN(iBytesRead);
}

int USSHSocket::send(const void* pData, int iDataLen)
{
   ...
   else if (iBytesWrite < 0) U_DUMP("status = %.*S", 512, status(iBytesWrite))
#endif
