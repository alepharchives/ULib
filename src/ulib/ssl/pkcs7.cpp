...
UPKCS7::~UPKCS7()
{
   ...
   if (indata) (void) U_SYSCALL(BIO_free, "%p", indata);
}
...
next:
   ...
   pkcs7 = (PKCS7*) ((U_STRNCMP(format, "PEM") == 0) ? U_SYSCALL(PEM_read_bio_PKCS7, "%p,%p,%p,%p", in, 0, 0, 0)
...
unsigned UPKCS7::getSignerCertificates(UVector<UCertificate*>& vec, int flags) const
{
   ...
   if (signers)
      ...
      X509* x;
      unsigned l = vec.size(),
               n = U_SYSCALL(sk_X509_num, "%p", signers);
...
PKCS7* UPKCS7::sign(const UString& data, const UString& signcert, const UString& pkey, const UString& passwd, const UString& certs, int flags)
{
   ...
   EVP_PKEY* key = UServices::loadKey(pkey, "PEM", true, passwd.c_str(), 0);
